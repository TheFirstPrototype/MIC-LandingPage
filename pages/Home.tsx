      ...
      {images.map((img, idx) => (
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
            <h1 className="text-4xl md:text-6xl font-extrabold text-[#FFCB05] mb-4 drop-shadow-lg uppercase tracking-tight">
              {img.title}
            </h1>
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
        {images.map((_, idx) => (
          <button
            ...
            className={`w-3 h-3 rounded-full ${idx === current ? 'bg-[#FFCB05]' : 'bg-white/50'}`}
          />
        ))}
      </div>
    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="md:w-1/2">
            <img 
              ...
              className="rounded-2xl shadow-2xl border-4 border-[#FFCB05]/30"
            />
          </div>
        </div>
      </section>
