...
const SponsorCard: React.FC<{ tier: string; price: string; features: string[]; available: string }> = ({ tier, price, features, available }) => (
  <div className="bg-white border-2 border-gray-100 rounded-3xl p-8 flex flex-col shadow-lg hover:border-[#C6DA31] transition-all relative overflow-hidden">
    <div className="absolute top-0 right-0 bg-[#C6DA31] text-[#112E4A] px-4 py-1 text-xs font-bold uppercase rounded-bl-xl">
      {available} Available
    </div>
    <h3 className="text-2xl font-bold text-[#112E4A] mb-2">{tier}</h3>
    <p className="text-3xl font-black text-[#112E4A] mb-6">{price}</p>
    ...
    <button className="w-full bg-[#112E4A] text-[#C6DA31] py-3 rounded-xl font-bold hover:bg-[#003d7c] transition-colors">
      Select {tier}
    </button>
  </div>
);
  ...
  return (
    <div className="animate-in fade-in duration-700">
      <div className="bg-[#112E4A] py-20 text-white text-center">
        <h1 className="text-5xl font-bold mb-4 text-[#C6DA31]">Sponsor & Volunteer</h1>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-3 mb-10">
            <DollarSign className="text-[#112E4A]" size={36} />
            <h2 className="text-4xl font-bold text-[#112E4A]">Sponsorship Packages</h2>
          </div>
          ...
          <div id="volunteer" className="bg-white p-12 rounded-[3rem] shadow-2xl border-4 border-[#C6DA31]/20 overflow-hidden relative">
            <div className="absolute top-0 right-0 p-8 opacity-5">
              <HandHeart size={200} className="text-[#112E4A]" />
            </div>
            <div className="relative z-10 max-w-3xl">
              <div className="flex items-center gap-3 mb-6">
                <CalendarCheck className="text-[#112E4A]" size={36} />
                <h2 className="text-4xl font-bold text-[#112E4A]">Join Our Volunteer Team</h2>
              </div>
